...
```
开源/竞品研究 → 需求探索 → 设计方案 → 实施规划 → 编码实现 → 部署体验 → 测试验证 → 代码审查 → 设计优化 → 交付
    ↑                                                                                    ↓
    └──────────────────── 每个阶段都可以回到前面修正 ────────────────────────────────────────┘
```
...
开源研究和竞品分析应当在项目启动之初、需求探索之前完成。原因：

- **避免重复造轮子**：先了解市场上已有的方案，才能判断是否值得做、怎么做
- **借鉴成熟设计**：站在巨人的肩膀上，把别人验证过的好模式纳入自己的设计
- **规避已知陷阱**：废弃项目的死因是最有价值的输入，能帮助在需求阶段就排除不可行的路线
- **明确差异化定位**：知道竞品做了什么，才能定义"我们做什么别人没做的"
...
将需求探索的成果固化为完整的、无歧义的设计文档，作为后续所有阶段的唯一参考。
...
2. **结构化设计文档**
  - 架构图（模块划分、数据流）
  - 接口定义（TypeScript 类型签名）
  - 配置方案（CLI 参数、环境变量、配置文件）
  - 错误处理策略
  - 测试策略
3. **自审检查清单**
  - 占位符扫描（TBD/TODO）
...
- **Feature Branch**：每个功能或任务组在独立分支上开发，完成后通过 PR 合入主分支
- **Git Worktree**：需要同时开发多个独立功能时，用 `git worktree` 创建隔离工作区，避免频繁 stash/切换
- 分支命名规范：`feature/<topic>`、`fix/<issue-id>`、`refactor/<scope>`

### 设计回溯路径

- 编码过程中发现设计方案有问题时，**不要硬写**
- 暂停编码，回到阶段二或阶段三修正设计/计划，再继续
- 记录回溯原因，作为设计阶段的改进输入

### 与 TDD 的衔接

- 实施规划中每个任务已包含测试用例，编码阶段严格按"红 → 绿 → 重构"节奏执行
- 先运行测试确认红（失败），再写实现确认绿（通过），最后重构保持绿
- 如果实现后测试仍失败，优先检查实现而非修改测试
...
## 阶段八：设计优化（VoltAgent/awesome-design-md 方法）
...
- 响应时间基线：首次请求 < 目标值，后续请求 < 目标值（具体值根据项目定义）
- 并发 SSE 连接：明确支持的并发数上限，超限时优雅降级
- 内存占用：空闲态、满载态的内存基线，无内存泄漏（长时间运行验证）
...
| 环节     | 工具                              | 用途               |
| ------ | ------------------------------- | ---------------- |
| 竞品分析   | gh api + WebSearch              | 验证项目状态和功能对比      |
