...
import {
  ...
  DEEPSEEK_API_CREATE_POW_CHALLENGE,
...
import type { DeepSeekPowChallenge } from '../../../src/providers/deepseek/pow.js';
...
const FAKE_CHALLENGE: DeepSeekPowChallenge = {
  algorithm: 'sha256',
  challenge: 'abc',
  difficulty: 8,
  salt: 'salt',
...
async function runChat(messages: ChatRequest['messages'], opts: { tools?: ChatRequest['tools'] } = {}): Promise<{
  ...
  const page = mockPage(async (args: unknown) => {
    ...
    if ((args as any).endpoint === DEEPSEEK_API_CREATE_POW_CHALLENGE) return { challenge: FAKE_CHALLENGE };
...
describe('deepseek outbound surface (allowlist enforcement)', () => {
  ...
  it('fetches the PoW challenge at the allowlisted endpoint', async () => {
    const { pageCalls } = await runChat([{ role: 'user', content: 'hi' }]);
    expect(pageCalls[1].endpoint).toBe(DEEPSEEK_API_CREATE_POW_CHALLENGE);
  ...
  it('performs completion at the absolute, allowlisted endpoint', async () => {
    const { pageCalls } = await runChat([{ role: 'user', content: 'hi' }]);
    expect(pageCalls[2].endpoint).toBe(DEEPSEEK_API_COMPLETION);
    expect(pageCalls[2].endpoint.startsWith('https://chat.deepseek.com')).toBe(true);
    expect(DEEPSEEK_ALLOWED_HOSTS).toContain(new URL(pageCalls[2].endpoint).hostname);
  });
  ...
  it('user/model-provided URLs can never become outbound destinations', async () => {
    ...
    expect(pageCalls[1].endpoint).toBe(DEEPSEEK_API_CREATE_POW_CHALLENGE);
    expect(pageCalls[2].endpoint).toBe(DEEPSEEK_API_COMPLETION);
    expect(pageCalls[2].endpoint).not.toContain(evil);
    // The evil URL travels only inside the prompt body — never as a target.
    expect(pageCalls[2].body.prompt).toContain(evil);
    // And the request still completes normally.
    expect(events.at(-1)).toMatchObject({ type: 'done', reason: 'stop' });
  });

  it('sends the bearer only on the three deepseek endpoints via the Authorization header', async () => {
    const { pageCalls } = await runChat([{ role: 'user', content: 'hi' }]);
    expect(pageCalls[0].bearerToken).toBe('test-token');
    expect(pageCalls[1].bearerToken).toBe('test-token');
    expect(pageCalls[2].bearerToken).toBe('test-token');
    expect(new URL(pageCalls[2].endpoint).hostname).toBe('chat.deepseek.com');
    expect(new URL(pageCalls[2].endpoint).hostname).toBe('chat.deepseek.com');
  });
